...
sizeCategoryData = pd.DataFrame(df.groupby(["mw_size_range",'local_permit_status']).agg({'project_mw':'sum','data_id':'count'}).reset_index()).rename(columns={'data_id':'project_count'})
